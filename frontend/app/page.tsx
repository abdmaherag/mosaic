...
export default function Home() {
  ...
  return (
    <div className="h-screen flex overflow-hidden bg-rag-gray-50">
      {/* Sidebar */}
      <aside
        className={`${
          sidebarOpen ? "w-72" : "w-0"
        } bg-white border-r border-rag-gray-200 flex flex-col flex-shrink-0 transition-all duration-300 overflow-hidden`}
      >
              ...
              <div key={cat} className="mb-1">
                ...
                {docs.map((doc) => {
                  ...
                  return (
                    <div
                      key={doc.id}
                      className="flex items-center gap-2 px-3 py-2 mx-1 rounded-lg hover:bg-rag-gray-50 transition-colors group"
                    >
                      <button
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 text-rag-gray-400 hover:text-red-500 transition-all"
                        title="Delete"
                      >
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-14 bg-white border-b border-rag-gray-200 flex items-center justify-between px-6 flex-shrink-0">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="p-1.5 hover:bg-rag-gray-100 rounded-lg transition-colors"
            >
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="hidden sm:inline text-xs text-rag-gray-400">
        ...
        <div ref={chatRef} className="flex-1 overflow-y-auto scrollbar-thin p-6 space-y-6">
          {exchanges.length === 0 ? (
            <div className="max-w-3xl mx-auto text-center py-16 animate-fade-in">
              ...
              <p className="text-rag-gray-500 mb-8 max-w-md mx-auto">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {[
                  ...
                  <button
                    key={p}
                    onClick={() => setPrompt(p)}
                    className="px-4 py-2 text-sm bg-white border border-rag-gray-200 rounded-lg hover:border-rag-blue-300 hover:bg-rag-blue-50 text-rag-gray-600 hover:text-rag-blue-700 transition-all"
                  >
        ...
        <div className="border-t border-rag-gray-200 bg-white p-4 flex-shrink-0">
          <div className="max-w-3xl mx-auto">
            <div className="relative border border-rag-gray-200 rounded-xl shadow-sm focus-within:ring-2 focus-within:ring-rag-blue-500 focus-within:border-transparent transition-all">
              <textarea
                ...
                className="w-full px-4 py-3 pr-14 text-sm text-black bg-transparent resize-none focus:outline-none scrollbar-thin max-h-32"
                style={{ minHeight: 48 }}
              />
              <div className="absolute right-2 bottom-2">
                <button
                  ...
                  className="p-2 bg-rag-blue-600 hover:bg-rag-blue-700 text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center justify-between mt-2 px-1">
              <div className="flex items-center gap-2 text-xs text-rag-gray-400">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span>
                  Querying across {activeCount} document{activeCount !== 1 ? "s" : ""}
                </span>
              </div>
              <span className="text-xs text-rag-gray-400">{input.length}/2000</span>
            </div>
          </div>
        </div>
      </main>
      ...
      {toast && (
        <div
          ...
          } text-white px-4 py-2 rounded-lg shadow-lg text-sm flex items-center gap-2 z-50 animate-slide-in`}
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            {toast.type === "success" ? (
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
  ...
  return (
    <div className="max-w-3xl mx-auto animate-slide-in">
      <div className="bg-white rounded-xl border border-rag-gray-200 shadow-sm overflow-hidden">
        {/* User query */}
        <div className="px-5 py-4 bg-rag-blue-50/50 border-b border-rag-gray-100">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-rag-gray-900">{ex.prompt}</p>
            </div>
          </div>
        </div>
        ...
        <div className="border-b border-rag-gray-100">
          <button
            ...
            className="w-full flex items-center gap-2 px-5 py-2.5 hover:bg-rag-gray-50 transition-colors text-left"
          >
            <div className="thinking-badge px-2 py-0.5 rounded text-[10px] font-semibold text-amber-700">
              THINKING
            </div>
              ...
              {THINKING_STEPS.map((step, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-2 py-1">
                    ...
                    <span className="text-xs text-rag-gray-600">{step}</span>
                    <span
        ...
        <div className="px-5 py-4">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-5 h-5 bg-rag-blue-600 rounded flex items-center justify-center">
              <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="prose prose-sm max-w-none text-rag-gray-700 text-sm leading-relaxed">
              ...
              <div className="space-y-2">
                {ex.sources.map((s, i) => (
                  <details key={i} className="group">
                    <summary className="cursor-pointer text-xs text-rag-blue-700 hover:text-rag-blue-900 flex items-center gap-2">
                      <span className="font-medium">{s.document}</span>
                      {s.pages.length > 0 && (
                        <span className="text-rag-gray-400">pp. {s.pages.join(", ")}</span>
                      )}
                      <span className="ml-auto text-rag-gray-400">score {s.score.toFixed(3)}</span>
                    </summary>
                    <div className="mt-2 pl-3 border-l-2 border-rag-blue-100 space-y-2">
                      {s.chunks.map((c, j) => (
                        <div key={j} className="text-xs text-rag-gray-600 bg-rag-gray-50 p-2 rounded">
                          {c.text}
                        </div>
                      ))}
                    </div>
        ...
        {/* Actions */}
        <div className="px-5 py-2.5 bg-rag-gray-50 border-t border-rag-gray-100 flex items-center gap-2">
          <button
